...
digitsOfInt n | n < 10 = [n]
              | otherwise = digitsOfInt (div (n - digit) 10) ++ [digit]
              where
                digit = mod n 10
...
additivePersistence n | n < 10 = 0
                      | otherwise = 1 + additivePersistence s
                        where
                            s = sumList (digitsOfInt n)
...
palindrome s | (listReverse s) == s = True
             | otherwise = False
